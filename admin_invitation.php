        ...
        <tr>
            <td>
				...
				<tr>
                    ...
                    <a href='http://192.168.3.214/Invitation_Portal_local/user_invitation_list.php?param=". $encryption ."&id=".$eventId."'><div style='display: inline-block;box-shadow: 0 0 14px rgb(126, 138, 143); padding:10px;'><span style='color: black;font-size: 20px;'>View Invitation<span></div></a>
					</td>
				</tr>
...
";
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'girmitidevtest@gmail.com';
        $mail->Password = 'Girmiti01';
        $mail->SMTPSecure = 'tls';
        $mail->Port       = 587;
        $mail->setFrom($from, "Girmiti Admin");
        $mail->addAddress($to, $membername);
        $mail->WordWrap = 50;
        $mail->isHTML(true);
        $mail->Subject = 'Girmiti Invitation';
        $mail->Body    = $txt;
        $retval = $mail->Send();
        if(!$retval){
            $status = 0;
